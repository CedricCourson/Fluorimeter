...
void setup() {
  ...
  } else
  {
    ...
    // modifier le code ici pour lire GPS et autre capteur quo n'ont besoin d'être lu qu'une seule fois (GPS), bat, Tint
    // puis les capteurs qui doivent être lu plusieur fois (F, T, P, S)+ rtc
    ...
    for (int i = 0; i < nbrMes; i++) {
      ...
      delay(delay_lec);
      battery_gauge();          // lecture des valeurs de la batterie
      delay(delay_lec);
      ...
      datachain += FuelGauge.percent(); datachain += ";"; datachain += FuelGauge.voltage() ; datachain += ";";
